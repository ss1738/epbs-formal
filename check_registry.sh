...
for f in "${SPECS[@]}"; do
  ...
  done < <(grep -oE '^(VAC_[A-Za-z0-9_]+|RVAC_[A-Za-z0-9_]+|S[0-9]_[A-Za-z0-9_]+|P[0-9][A-Za-z0-9_]*|TypeOK|AncClosure|AncRootsUnique|StructuralClosure|AdversaryBudget|MonotoneHistory)\b' "$f" \
           | sort -u)
done
