...
- **Fork-choice structure matches.** Gloas gates a withholding reorg on head-weakness and parent-strength weight thresholds, the same accumulated-weight comparison `EPBSForkChoice.tla` modelled (`coq/EPBSForkChoice.v` is DELETED -- it proved a threshold over PayloadBoost, a quantity Gloas does not have; see D5_PAYLOAD_WEIGHT.md); Gloas leaves the exact numeric thresholds to committee fractions inherited from prior forks.
